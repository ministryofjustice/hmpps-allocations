...
class CommunityApiClient(private val webClient: WebClient) {
  ...
  fun getInductionContacts(crn: String, contactDateFrom: LocalDate): Mono<List<Contact>> {
    ...
    return webClient
      ...
      .onStatus(
        { httpStatus -> HttpStatus.NOT_FOUND == httpStatus },
        { Mono.error(MissingInductionError("No induction countact found for $crn")) }
      )
      .bodyToMono(responseType)
      .onErrorResume { ex ->
        when (ex) {
          is MissingInductionError -> Mono.just(emptyList())
          else -> Mono.error(ex)
        }
      }
  }
...
private class MissingInductionError(msg: String) : RuntimeException(msg)
