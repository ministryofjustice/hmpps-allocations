...
internal class GetUnallocatedCaseServiceTest {
  ...
  fun `must not return unallocated cases which get deleted during enrichment`() {
    ...
    every { mockWorkforceAllocationsToDeliusApiClientClient.getDeliusCaseDetails(listOf(unallocatedCaseEntity)) } returns Mono.just(emptyList())
    val cases = GetUnallocatedCaseService(mockRepo, mockCommunityClient, mockk(), mockk(), mockk(), EnrichEventService(mockCommunityClient, mockWorkforceAllocationsToDeliusApiClientClient, mockk(), mockRepo), mockk()).getAllByTeam("TM1").collectList().block()
